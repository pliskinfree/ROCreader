...
struct PdfRuntime::Impl {
  ...
  void StartPrefetchQueues() {
    ...
    for (int i = 0; i < requested_lanes; ++i) {
      const std::string thread_name = "pdf_prefetch_worker_" + std::to_string(i + 1);
  ...
  bool WantsIdlePrefetch(Uint32 now) const {
    ...
    if (can_prefetch_during_target && !HasIdlePrefetchLane()) return false;
    if (!can_prefetch_during_target && prefetch_active) return false;
...
bool PdfRuntime::CanDrawPageAt(int page_index) const {
  return impl_ ? impl_->CanDrawPageAt(page_index) : false;
}

void PdfRuntime::PrefetchPageAt(int page_index) {
