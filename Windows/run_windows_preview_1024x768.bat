...
"%BASH_EXE%" -lc "export PATH=/ucrt64/bin:/usr/bin:$PATH; PROJECT_UNIX=$(cygpath -u \"$PROJECT_WIN\"); cd \"$PROJECT_UNIX\"; export ROCREADER_ROOT=\"$PROJECT_UNIX\"; if [ -f \"$PROJECT_UNIX/URL/online_sources.ini\" ]; then export ROCREADER_ONLINE_SOURCES=\"$PROJECT_UNIX/URL/online_sources.ini\"; echo \"[INFO] URL source config: $ROCREADER_ONLINE_SOURCES\"; fi; if command -v make >/dev/null 2>&1; then MK=make; elif command -v mingw32-make >/dev/null 2>&1; then MK=mingw32-make; else echo '[ERROR] make not found'; exit 10; fi; if ! command -v g++ >/dev/null 2>&1; then echo '[ERROR] g++ not found (install mingw-w64-ucrt-x86_64-toolchain)'; exit 11; fi; if ! pkg-config --exists sdl2; then echo '[ERROR] SDL2 dev package missing'; exit 12; fi; if ! pkg-config --exists SDL2_mixer; then echo '[WARN] SDL2_mixer dev package missing, will fallback to SDL audio backend.'; echo '[INFO] Optional install:'; echo '  pacman -S --needed mingw-w64-ucrt-x86_64-SDL2_mixer'; fi; echo '[INFO] build (REQUIRE_MUPDF=1)'; $MK clean TARGET=Windows/build/rocreader_sdl.exe; $MK -j4 REQUIRE_MUPDF=1 TARGET=Windows/build/rocreader_sdl.exe; RC=$?; if [ $RC -ne 0 ]; then exit $RC; fi; echo '[INFO] launch 1024x768 preview'; rm -f Windows/preview_1024x768.log; set -o pipefail; ROCREADER_LOG_STDERR=1 ROCREADER_VERBOSE_LOG=1 ROCREADER_WINDOWED=1 ROCREADER_SCREEN_PROFILE=1024x768 ROCREADER_SCREEN_W=1024 ROCREADER_SCREEN_H=768 ./Windows/build/rocreader_sdl.exe 2>&1 | tee Windows/preview_1024x768.log"

set "RC=%ERRORLEVEL%"
echo.
if not "%RC%"=="0" (
  echo [ERROR] 1024x768 preview failed, exit code %RC%.
  pause
  exit /b %RC%
)

echo [INFO] 1024x768 preview exited normally.
pause
exit /b 0
