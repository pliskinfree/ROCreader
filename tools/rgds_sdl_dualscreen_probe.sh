#!/bin/sh
# RGDS dual-screen SDL/KMSDRM visual probe.
# Run on RGDS:
#   sh rgds_sdl_dualscreen_probe.sh
# Optional:
#   ROCREADER_RGDS_DISPLAY_TEST_SECONDS=15 sh rgds_sdl_dualscreen_probe.sh
#
# This script is self-contained: it first tries a sibling prebuilt binary, then
# tries to compile a tiny SDL2 probe into /tmp and run it. If the target system
# has no compiler, the script exits with a clear reason while still being useful
# when called from roc_matrix_probe.sh.
...
SECONDS_TO_RUN="${ROCREADER_RGDS_DISPLAY_TEST_SECONDS:-15}"
...
compiler=""
for c in g++ c++ clang++ aarch64-linux-gnu-g++; do
  if have_cmd "$c"; then
    compiler="$c"
    break
  fi
...
if have_cmd pkg-config; then
  ...
  sdl_libs=$(pkg-config --libs sdl2 2>/dev/null || printf '%s' "-lSDL2")
elif have_cmd sdl2-config; then
  sdl_cflags=$(sdl2-config --cflags 2>/dev/null || true)
  sdl_libs=$(sdl2-config --libs 2>/dev/null || printf '%s' "-lSDL2")
fi
...
if ! "$compiler" -O2 -std=c++17 $sdl_cflags "$TMP_CPP" -o "$TMP_BIN" $sdl_libs >> "$LOG_FILE" 2>&1; then
  ...
  log "[rgds_probe_sh] sdl_libs=$sdl_libs"
  exit 126
fi

chmod +x "$TMP_BIN" 2>/dev/null || true
log "[rgds_probe_sh] running freshly built probe: $TMP_BIN"
run_probe_bin "$TMP_BIN"
