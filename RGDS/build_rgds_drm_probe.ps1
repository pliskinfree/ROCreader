...
aarch64-linux-gnu-g++ -O2 -std=c++17 -Wall -Wextra \
  ...
  /work/RGDS/src/rgds_drm_dualscreen_probe.cpp \
  -o /out/rgds_drm_dualscreen_probe \
  -L/work/H700/sysroot_device/usr/lib/aarch64-linux-gnu \
  -ldrm
file /out/rgds_drm_dualscreen_probe
aarch64-linux-gnu-readelf -d /out/rgds_drm_dualscreen_probe | sed -n '1,120p'
"@
...
$Built = Join-Path $OutDirAbs "rgds_drm_dualscreen_probe"
if (-not (Test-Path $Built)) {
    throw "Build did not produce: $Built"
}

if (Test-Path $SdAppsDir) {
    $DevicePayloadDir = Join-Path $SdAppsDir ".rgds_drm_probe_files"
    New-Item -ItemType Directory -Force -Path $DevicePayloadDir | Out-Null
    Copy-Item -LiteralPath $Built -Destination (Join-Path $DevicePayloadDir "rgds_drm_dualscreen_probe") -Force
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot "rgds_drm_dualscreen_probe.sh") -Destination (Join-Path $SdAppsDir "rgds_drm_dualscreen_probe.sh") -Force
    Write-Host "Copied RGDS DRM probe to $SdAppsDir"
} else {
    Write-Host "SD apps dir not found, built DRM probe left at $Built"
}

Write-Host "RGDS DRM dual-screen probe built: $Built"
