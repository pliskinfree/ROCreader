...
$Source = Join-Path $RepoRootAbs "tools\rgds_weston_spanning_probe.cpp"
...
aarch64-linux-gnu-g++ -O2 -std=c++17 -Wall -Wextra \
  ...
  -D_REENTRANT \
  /work/tools/rgds_weston_spanning_probe.cpp \
  -o /out/rgds_weston_spanning_probe \
  -L/work/H700/sysroot_device/usr/lib/aarch64-linux-gnu \
  -lSDL2 -ldl -lpthread
file /out/rgds_weston_spanning_probe
aarch64-linux-gnu-readelf -d /out/rgds_weston_spanning_probe | sed -n '1,120p'
"@
...
$Built = Join-Path $OutDirAbs "rgds_weston_spanning_probe"
if (-not (Test-Path $Built)) {
    throw "Build did not produce: $Built"
}

if (Test-Path $SdAppsDir) {
    $DevicePayloadDir = Join-Path $SdAppsDir ".rgds_weston_spanning_probe_files"
    New-Item -ItemType Directory -Force -Path $DevicePayloadDir | Out-Null
    Copy-Item -LiteralPath $Built -Destination (Join-Path $DevicePayloadDir "rgds_weston_spanning_probe") -Force
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot "rgds_weston_spanning_probe.sh") -Destination (Join-Path $SdAppsDir "rgds_weston_spanning_probe.sh") -Force
    Write-Host "Copied RGDS Weston spanning probe to $SdAppsDir"
} else {
    Write-Host "SD apps dir not found, built probe left at $Built"
}

Write-Host "RGDS Weston spanning probe built: $Built"
